...
**6. Persistence layer**
ElastiCache Serverless (Valkey/Redis) is currently used as the primary persistence layer, which works well for a URL shortener given the read heavy access pattern. In a more complete production setup it would be more appropriate to use Redis purely as a cache and ratelimit counter, with a relational database (RDS PostgreSQL) as the source of truth for links, users, and analytics. This would improve durability guarantees and make complex queries (e.g. analytics aggregation across users) significantly easier.
 
**7. Scaling**
ECS is currently deployed with minimal resources and a fixed task count. To improve reliability and cost efficiency, it should use auto scaling (via Amazon Elastic Container Service Application Auto Scaling) based on metrics such as CPU utilization or ALB request load, with defined minimum and maximum task limits. For caching, Amazon ElastiCache (Valkey) should rely on its serverless auto-scaling capabilities. Overall, implementing dynamic scaling ensures the system can handle traffic spikes while avoiding unnecessary resource costs during low usage.